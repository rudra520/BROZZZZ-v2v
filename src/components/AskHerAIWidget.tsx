  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'widget-welcome',
      role: 'assistant',
      content: "Hello! I am **Ask-Her-AI**, your 24/7 STEM Career & Mentorship ally. I am trained to review engineering resumes, optimize negotiation dialogues, calibrate technical portfolio statements, and help you conquer STEM hurdles. How can I guide you today?",
  ...
  const renderFormattedText = (text: string) => {
    return text.split('\n').map((line, lineIdx) => {
      ...
      while ((match = boldRegex.exec(content)) !== null) {
        ...
        parts.push(
          <strong key={match.index} className="text-white font-bold bg-white/10 px-1 rounded">
            {match[1]}
          </strong>
        );
      ...
      if (isBullet) {
        return (
          <div key={lineIdx} className="flex gap-1.5 items-start ml-2 my-1 text-[11px] text-slate-300 leading-relaxed">
            <span className="text-pink-400 mt-1.5 flex-shrink-0 w-1.5 h-1.5 rounded-full bg-pink-400" />
      ...
      return (
        <p key={lineIdx} className={`${line.trim() === '' ? 'h-2' : 'my-1'} text-[11px] text-slate-300 leading-relaxed`}>
          {renderedLine}
        </p>
      );
    });
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] text-left" id="ask-her-ai-mentor-widget">
      ...
      >
        ...
        <span className="absolute inset-0 rounded-full bg-gradient-to-tr from-pink-500 via-purple-600 to-violet-600 animate-pulse blur-[8px] opacity-75 group-hover:opacity-100 transition-opacity" />
        
        {/* Real Avatar Container */}
        <div className="relative w-14 h-14 rounded-full bg-[#0B0F17] flex items-center justify-center border border-white/20 text-white overflow-hidden">
          {isOpen ? (
            <X size={22} className="text-slate-200" />
          ) : (
            <div className="relative flex items-center justify-center w-full h-full">
              {/* Spinning gradient border */}
              <div className="absolute inset-0.5 rounded-full bg-gradient-to-tr from-pink-500 to-violet-600 animate-spin opacity-40 blur-[1px] pointer-events-none" />
              <MessageSquare size={22} className="text-pink-400 animate-pulse relative z-10" />
              <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-emerald-500 rounded-full border border-[#0B0F17] animate-ping" />
              <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-emerald-500 rounded-full border border-[#0B0F17]" />
            </div>
          )}
        </div>
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-20 right-0 w-[360px] sm:w-[420px] max-h-[580px] bg-[#0E1320]/90 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl shadow-black/80 flex flex-col justify-between overflow-hidden"
            id="chat-modal-panel"
          >
            {/* Header section */}
            <div className="px-5 py-4 bg-slate-950/60 border-b border-white/10 flex items-center justify-between relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-pink-500 via-purple-500 to-violet-500" />
              
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-pink-500 to-purple-600 flex items-center justify-center border border-white/15 shadow-inner">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-pink-400 font-mono tracking-wide mt-1">
            ...
            >
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`rounded-xl p-3.5 text-xs relative ${
                      isUser
                        ? 'bg-purple-500/20 border border-purple-500/30 text-slate-100 rounded-tr-none shadow-md shadow-purple-500/5'
                        : 'bg-slate-900/80 border border-pink-500/20 text-slate-100 rounded-tl-none shadow-md shadow-pink-500/5'
                    }`}>
                      {/* Left glowing marker for AI response */}
                      {!isUser && (
                        <div className="absolute top-0 bottom-0 left-0 w-[3px] bg-gradient-to-b from-pink-500 to-purple-600 rounded-l" />
                      )}

                      <div className="text-left font-sans select-text">
                        {renderFormattedText(msg.content)}
                      </div>
                      <span className="block text-[8px] text-slate-500 font-mono mt-1.5 tracking-wider uppercase text-right">
                        {msg.timestamp}
                      </span>
                    </div>
                  </div>
              ...
              {loading && (
                <div className="flex gap-2.5 max-w-[85%] text-left">
                  <div className="w-6.5 h-6.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 flex items-center justify-center animate-spin">
                    <Sparkles size={11} />
                  </div>
                  <div className="rounded-xl p-3 bg-slate-900/60 border border-white/5 rounded-tl-none text-[11px] text-slate-400 flex items-center gap-1.5 shadow-md">
              ...
              {isListening && (
                <div className="flex gap-2.5 max-w-[85%] text-left">
                  <div className="w-6.5 h-6.5 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-400 flex items-center justify-center animate-pulse">
                    <Mic size={11} />
                  </div>
                  <div className="rounded-xl p-3 bg-pink-500/5 border border-pink-500/20 rounded-tl-none text-[11px] text-slate-300 flex flex-col gap-2 shadow-md">
                    <span className="font-mono text-[9px] uppercase font-bold text-pink-400 animate-pulse">
            ...
            <div className="px-4 py-2 border-t border-white/5 bg-slate-950/40 text-left">
              <span className="text-[9px] uppercase font-bold text-slate-500 tracking-wider font-mono block mb-1.5">
              ...
              <div className="flex flex-col gap-1.5">
                {FLOATING_CHIPS.map((chip, idx) => (
                  <button
                    ...
                    className="w-full text-left text-[10px] text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 border border-white/5 hover:border-purple-500/30 px-3 py-1.5 rounded-lg transition-all truncate duration-100 disabled:opacity-50"
                  >
                    🚀 {chip}
                  </button>
                ))}
            ...
            <div className="p-3 bg-slate-950/80 border-t border-white/10 space-y-2">
              {/* Attachment Pill Indicator */}
              {attachedFile && (
                <div className="flex items-center justify-between px-3 py-1 bg-purple-500/10 border border-purple-500/30 rounded-lg text-xs">
              ...
              >
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full pl-3 pr-8 py-2 bg-slate-900 border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:border-purple-500 transition-all placeholder:text-slate-600 disabled:opacity-50"
                  />
                ...
                <button
                  ...
                  className="p-2 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-xl hover:opacity-95 active:scale-95 transition-all shadow-md shadow-purple-500/10 disabled:opacity-50 disabled:active:scale-100 flex-shrink-0"
                  title="Send message"
                >
                  <Send size={14} />
                </button>
